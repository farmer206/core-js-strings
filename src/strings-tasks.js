...
function removeFirstOccurrences(str, value) {
  ...
  const end = str.indexOf(value);
  const start = value.length + end;
  return str.substring(0, end) + str.substring(start);
}
...
function removeLastOccurrences(str, value) {
  ...
  const end = str.lastIndexOf(value);
  const start = value.length + end;
  return str.substring(0, end) + str.substring(start);
}
...
function isPalindrome(str) {
  const regulStr = str.toLowerCase().replace(/\s|\p{P}/gu, '');
  const reverStr = regulStr.split('').reverse().join('');
  return regulStr === reverStr;
}
...
function extractNameFromTemplate(value) {
  return value.replace(/Hello|\p{P}/gu, '').trim();
}
...
function encodeToRot13(str) {
  return str.replace(/[a-z]/gi, (item) =>
    String.fromCharCode(
